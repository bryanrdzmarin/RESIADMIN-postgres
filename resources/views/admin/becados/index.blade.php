<x-layouts.app >
    ...
    <div class="mb-5 flex justify-between items-center">
        ...
        <div class="flex gap-x-3 mr-8">
            <form action="{{ route('admin.becados.show') }}" method="GET">
    ...
    <flux:radio.group label="" variant="segmented" size="sm" class="w-fit mx-auto mb-4">
        <flux:radio value="todos" class="{{ request()->routeIs('admin.becados.index')|| request()->routeIs('admin.becados.show') ? 'bg-white text-black' : '' }}">
            <a href="{{ route('admin.becados.index') }}" class="block w-full text-center font-semibold text-sm">
                Todos
            </a>
        </flux:radio>
        
        <flux:radio value="disponibles" class="{{ request()->routeIs('admin.becados.indexNacionales') ? 'bg-white text-black' : '' }}">
                    ...
                    <tr class="odd:bg-white even:bg-gray-100">
                        ...
                        <td class="border border-gray-300 px-4 py-2 text-center">
                            ...
                            <div class="flex space-x-2 items-stretch">
                                <!-- Editar -->
                                <form action="{{ route('admin.becados.edit',['becado'=>$becado->ci, 'origen'=>'becados']) }}" method="GET">
                                    <button type="submit"
                                ...
                                <!-- Eliminar -->
                                <form  action="{{ route('admin.becados.destroy', [$becado->ci]) }}" method="POST"
