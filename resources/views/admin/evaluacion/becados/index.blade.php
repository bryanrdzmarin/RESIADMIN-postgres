<x-layouts.app >
    ...
    <div class="mb-5 flex justify-between items-center">
        ...
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Becados</flux:breadcrumbs.item>
        </flux:breadcrumbs>

        <div class="flex gap-x-3 mr-8">
            <form action="{{ route('admin.evaluar.becados.showbecadosEvaluacion') }}" method="GET">
   ...
   <flux:radio.group label="" variant="segmented" size="sm" class="w-fit mx-auto mb-4">
        <flux:radio value="todos" class="{{ request()->routeIs('admin.evaluar.becados.indexbecadosEvaluacion')|| request()->routeIs('admin.evaluar.becados.showbecadosEvaluacion') ? 'bg-white text-black' : '' }}">
        ...
        <flux:radio value="ocupados" class="{{ request()->routeIs('admin.evaluar.becados.indexbecadosExtranjerosEvaluacion') ? 'bg-white text-black' : '' }}">
            ...
            @else
                @foreach($becados as $becado)
                    <tr class="odd:bg-white even:bg-gray-100">
                        ...
                        <td class="border border-gray-300 px-4 py-2 text-center">{{ $becado->origen==='Extranjero' ? $becado->becadoExtranjero->evaluacion_jefe_relaciones_internacionales : 'No aplica' }}</td>
                        <td class="border border-gray-300 px-4 py-2 text-center">
                            
                            @if(is_null($becado->evaluacion_final))
                                <form action="{{ route('admin.evaluar.becados.evaluarBecado', [$becado->ci, 'origen' => 'becados']) }}" method="GET">
                        ...
                        <td class="border border-gray-300 px-4 py-2 text-center align-middle">
                            <div class="flex items-center justify-center">
                                <form action="{{ route('admin.evaluar.becados.editarEvaluacion', ['becado' => $becado->ci, 'origen' => 'becados']) }}" method="GET">
