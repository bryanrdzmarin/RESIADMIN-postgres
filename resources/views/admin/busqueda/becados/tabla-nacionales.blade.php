<table id="tabla-nacionales"
    class="border-collapse border border-gray-400 w-full text-left mb-4"
    style="{{ isset($busquedaOrigen) && $busquedaOrigen !== 'nacionales' ? 'display: none;' : '' }}">
    <thead>
        <tr class="bg-gray-200">
            ...
            <th class="border border-gray-300 px-4 py-2 text-center">Dirección</th>
    ...
    <tbody>
        ...
        @else
            @foreach($becados as $becadoNacional)
                <tr class="odd:bg-white even:bg-gray-100">
                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $becadoNacional->becado?->ci ?? '—' }}</td>
                    ...
                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $becadoNacional->becado?->apto?->numero ?? '—' }}</td>
                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $becadoNacional->becado?->residencia?->nombre ?? '—' }}</td>
                    ...
                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $becadoNacional->becado?->evaluacion_jefe_residencia ?? '—' }}</td>
                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $becadoNacional->becado?->evaluacion_jefe_apto ?? '—' }}</td>
                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $becadoNacional->becado?->evaluacion_profesor ?? '—' }}</td>
                    <td class="border border-gray-300 px-4 py-2 text-center">{{ $becadoNacional->becado?->evaluacion_final ?? '—' }}</td>
                </tr>
            @endforeach
        @endif
    </tbody>
